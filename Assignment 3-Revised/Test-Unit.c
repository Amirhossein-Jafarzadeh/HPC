...
considering interactions between individuals and
...
#define M 10000
#define N 10000
int n_s=100, n_z=100, n_f_s=100, n_f_z=100; /*fixing the number of
...
int main() {
  ...
  int i,j,r,k,t;
  int T_max=200;/*Initialization for number of movements*/
  ...
  for(int q=0;q<20;q++){/*repeating movement loop for different increasing number of maximum movements, 20 times*/
    gettimeofday(&start, NULL);
    for(t=0;t<T_max;t++) /*We move individuals T times in order of partition, from partition one to four depending on number of threads*/
      {
      for(i=0;i<n_s+n_z;i++)
        {
          /*/////////Moving interior individuals/////////////*/
          /*first if finds individuals in the first partition and moves them*/
          if(P[i].partition==1 && P[i].position=='I' && (n_thread==1 || n_thread==2 || n_thread==4))
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*second if finds individuals in the second partition and moves them*/
          if(P[i].partition==2 && P[i].position=='I' && (n_thread==2 || n_thread==4))
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*third if finds individuals in the third partition and moves them*/
          if(P[i].partition==3 && P[i].position=='I' && n_thread==4)
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*fourth if finds individuals in the fourth partition and moves them*/
          if(P[i].partition==4 && P[i].position=='I' && n_thread==4)
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }

          /*///////////Moving top boundary individuals///////////*/
          /*first if finds individuals in the first partition and moves them*/
          if(P[i].partition==1 && P[i].position=='T' && (n_thread==1 || n_thread==2 || n_thread==4))
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*second if finds individuals in the second partition and moves them*/
          if(P[i].partition==2 && P[i].position=='T' && (n_thread==2 || n_thread==4))
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*third if finds individuals in the third partition and moves them*/
          if(P[i].partition==3 && P[i].position=='T' && n_thread==4)
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*fourth if finds individuals in the fourth partition and moves them*/
          if(P[i].partition==4 && P[i].position=='T' && n_thread==4)
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }

          /*///////Moving bottom boundary individuals/////////*/
          /*first if finds individuals in the first partition and moves them*/
          if(P[i].partition==1 && P[i].position=='B' && (n_thread==1 || n_thread==2 || n_thread==4))
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*second if finds individuals in the second partition and moves them*/
          if(P[i].partition==2 && P[i].position=='B' && (n_thread==2 || n_thread==4))
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*third if finds individuals in the third partition and moves them*/
          if(P[i].partition==3 && P[i].position=='B' && n_thread==4)
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
          /*fourth if finds individuals in the fourth partition and moves them*/
          if(P[i].partition==4 && P[i].position=='B' && n_thread==4)
            {
            movement(P[i].index, i, n_s, n_z);
            partition(i, n_thread , n_s, n_z);
            }
    ...
    printf("\n%lu micro seconds for %d Movements\n", (stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec, T_max);
    T_max+=200; /*increasing number of movements for the next test*/
  }
  for(i=0;i<n_s+n_z;i++)
    {
    assign_dimensions(P[i].index,i);
...
int t;
  ...
  if(threads==2 && M%2==0)
    ...
    if(P[i].index>0 && P[i].index<=M*N/2 && M>1)
      P[i].partition=1;
    if(P[i].index>M*N/2-N && P[i].index<=M*N/2 && M>1)
      P[i].position='B';
    if(P[i].index>M*N/2 && P[i].index<=M*N/2+N && M>1)
      P[i].position='T';
  ...
  if(threads==2 && M%2!=0)
    ...
    if(P[i].index>0 && P[i].index<=(1+M/2)*N && M>1)
      P[i].partition=1;
    if(P[i].index>((1+M/2)*N)-N && P[i].index<=(1+M/2)*N && M>1)
      P[i].position='B';
    if(P[i].index>(1+M/2)*N && P[i].index<=((1+M/2)*N)+N && M>1)
      P[i].position='T';
  ...
  if(threads==4 && M%4==0)
   {
    ...
    if(P[i].index>0 && P[i].index<=M*N/4 && M>3)
      P[i].partition=1;
    if(P[i].index>(M*N/4)-N && P[i].index<=M*N/4 && M>1)
      P[i].position='B';
    if(P[i].index>M*N/4 && P[i].index<=(M*N/4)+N && M>1)
      P[i].position='T';
    if(P[i].index>M*N/4 && P[i].index<=2*(M*N/4) && M>3)
      P[i].partition=2;
    if(P[i].index>(2*(M*N/4))-N && P[i].index<=2*(M*N/4) && M>1)
      P[i].position='B';
    if(P[i].index>2*(M*N/4) && P[i].index<=(2*(M*N/4))+N && M>1)
      P[i].position='T';
    if(P[i].index>2*(M*N/4) && P[i].index<=3*(M*N/4) && M>3)
      P[i].partition=3;
    if(P[i].index>(3*(M*N/4))-N && P[i].index<=3*(M*N/4) && M>1)
      P[i].position='B';
    if(P[i].index>3*(M*N/4) && P[i].index<=(3*(M*N/4))+N && M>1)
      P[i].position='T';
  ...
  if(threads==4 && M%4==1)
    ...
    if(P[i].index>0 && P[i].index<=(1+M/4)*N && M>3)
      P[i].partition=1;
    if(P[i].index>((1+M/4)*N)-N && P[i].index<=(1+M/4)*N && M>1)
      P[i].position='B';
    if(P[i].index>(1+M/4)*N && P[i].index<=((1+M/4)*N)+N && M>1)
      P[i].position='T';
    if(P[i].index>(1+M/4)*N && P[i].index<=(1+M/4)*N+(M/4)*N && M>3)
      P[i].partition=2;
    if(P[i].index>((1+M/4)*N+(M/4)*N)-N && P[i].index<=(1+M/4)*N+(M/4)*N && M>1)
      P[i].position='B';
    if(P[i].index>(1+M/4)*N+(M/4)*N && P[i].index<=((1+M/4)*N+(M/4)*N)+N && M>1)
      P[i].position='T';
    if(P[i].index>(1+M/4)*N+M/4*N && P[i].index<=(1+M/4)*N+(2*M/4)*N && M>3)
      P[i].partition=3;
    if(P[i].index>((1+M/4)*N+(2*M/4)*N)-N && P[i].index<=(1+M/4)*N+(2*M/4)*N && M>1)
      P[i].position='B';
    if(P[i].index>(1+M/4)*N+(2*M/4)*N && P[i].index<=((1+M/4)*N+(2*M/4)*N)+N && M>1)
      P[i].position='T';
  ...
  if(threads==4 && M%4==2)
    ...
    if(P[i].index>(1+M/4)*N && P[i].index<=2*(1+M/4)*N && M>3)
      P[i].partition=2;
    if(P[i].index>(2*(1+M/4)*N)-N && P[i].index<=2*(1+M/4)*N && M>1)
      P[i].position='B';
    if(P[i].index>2*(1+M/4)*N && P[i].index<=(2*(1+M/4)*N)+N && M>1)
      P[i].position='T';
    if(P[i].index>2*(1+M/4)*N && P[i].index<=2*(1+M/4)*N+(M/4)*N && M>3)
      P[i].partition=3;
    if(P[i].index>(2*(1+M/4)*N+(M/4)*N)-N && P[i].index<=2*(1+M/4)*N+(M/4)*N && M>1)
      P[i].position='B';
    if(P[i].index>2*(1+M/4)*N+(M/4)*N && P[i].index<=(2*(1+M/4)*N+(M/4)*N)+N && M>1)
      P[i].position='T';
  ...
  if(threads==4 && M%4==3)
    ...
    if(P[i].index>2*(1+M/4)*N && P[i].index<=3*(1+M/4)*N && M>3)
      P[i].partition=3;
    if(P[i].index>(3*(1+M/4)*N)-N && P[i].index<=3*(1+M/4)*N && M>1)
      P[i].position='B';
    if(P[i].index>3*(1+M/4)*N && P[i].index<=(3*(1+M/4)*N)+N && M>1)
      P[i].position='T';
